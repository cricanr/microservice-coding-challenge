...
class RatingService @Inject()(ws: WSClient)
  ...
  private[services] def createOutputFile(ratingsFilePath: String): Try[OutputStream] = {
    Try {
  ...
  def streamRatingsToFile(): Unit = {
    ws.url("http://localhost:3020/ratings").withMethod("GET").stream().foreach { response =>
      ...
      tryOutputStream.map { outputStream =>
      ...
      }.recover { case failure: Throwable =>
        ...
        println(s"Failure reading streaming data from file: ${failure.getClass.getSimpleName}: ${failure.getMessage}")
      }
    }
  }

  private[services] def decodeRatingMessage(bytes: ByteString): Option[Rating] = {
  ...
  private def rating(movieId: Int, ratingsAverageList: Seq[Rating] = ratingAvgList): Option[Rating] = {
  ...
  def ratingsAverage(ratingLinesUnfiltered: Seq[String]): Seq[Rating] = {
    val maybeRatingsPruned = ratingLinesUnfiltered.collect { case line if line nonEmpty => line.trim }.collect { case line if line.length > 6 => line.substring(6) }

    val ratings = maybeRatingsPruned.map(line => decodeRating(line)).collect { case decodedRating  if decodedRating nonEmpty => decodedRating.get }
  ...
  def isBellowMedianRating(movieId: Int, ratingsAverageList: Seq[Rating]): Boolean = {
    val size = ratingsAverageList.size
    val averageRating = if (size == 0) -1 else ratingsAverageList.map(_.rating).sum / size

    val maybeRating = rating(movieId, ratingsAverageList)

    maybeRating.forall(rating => if (rating.rating < averageRating) true else false)
  }

  def ratingAverageInfo(movieId: Int, ratingsAverageList: Seq[Rating] = ratingAvgList): Option[RatingAverageInfo] = {
    val maybeRating = rating(movieId, ratingsAverageList)
    maybeRating.map(rating => RatingAverageInfo(rating, isBellowMedianRating(movieId, ratingsAverageList)))
  }

  def ratingAverageInfos(moviesInfo: MoviesInfo): Seq[RatingAverageInfo] = {
    moviesInfo.data.map(movieInfo => ratingAverageInfo(movieInfo.id)).collect { case ratingAverageInfo if ratingAverageInfo.nonEmpty => ratingAverageInfo.get }
  }
}
