...
trait IArtistInfoService {
  def artistInfo(artistInfoRequest: ArtistInfoRequest)(
      implicit ec: ExecutionContext): Future[ArtistInfos]
}

class ArtistInfoService @Inject()(requestHandler: IRequestHandler)(
    implicit ec: ExecutionContext,
    wSClient: WSClient)
    extends IArtistInfoService {
  ...
  def artistInfo(artistInfoRequest: ArtistInfoRequest)(
    ...
    implicit val retryConfig: RetryConfig =
      RetryConfig(noTries = 3, factor = 1.7f, 1, 0)
    callEndpointWithRetry[ArtistInfos](urlArtistInfo)
  }

  def artistInfos(artistIds: Seq[Int]): Future[Seq[ArtistInfos]] = {
    val artistInfoRequests = artistIds
      .grouped(5)
      .toSeq
      .map(artistIdBatch => ArtistInfoRequest(artistIdBatch, 5, 0))

    val artistInfoFutureResponses =
      artistInfoRequests.map(artistInfoRequest => artistInfo(artistInfoRequest))
