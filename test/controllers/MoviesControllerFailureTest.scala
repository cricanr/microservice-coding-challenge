...
class MoviesControllerFailureTest extends WordSpec with Matchers with MockitoSugar with GuiceOneAppPerSuite {
  ...
  "The MoviesController" when {
    "calling aggregate with a correct query" should {
      "respond the movies det" in {
        ...
        val fakeRequest = FakeRequest(method = "movies", "?genre=bam%20Fiction&revenue=500000000&offset=0&limit=10")
        val result = controller.movies()(fakeRequest.withHeaders())

        result.toString shouldEqual Future.failed(new Exception("Invalid search parameters supplied.")).toString
      }
    }
  }
}
