...
class RetryHandlerTest extends TestKit(ActorSystem("MySpec")) with WordSpecLike with BeforeAndAfterAll with Matchers with MockitoSugar with ScalaFutures with Eventually {
  private implicit val ec: ExecutionContextExecutor = ExecutionContext.global
  private implicit val wsClientMock: WSClient = mock[WSClient]

  private implicit def decoderSuccessMock(body: String): Either[circe.Error, String] = Right("success")
  ...
  "The RetryHandler" when {
    "calling callWithRetryWithoutArgs for function returning a failed future" should {
      "call function with retry back-off propagation in case of error" in {
        ...
        implicit val retryConfig: RetryConfig = RetryConfig(noTries = 2, factor = 1.5f, init = 1, cur = 0)
        val f = retryHandler.callWithRetry[String](serviceCallFailing)(uriFailure)

        eventually(f.toString shouldEqual Future.failed(new GenericFailureCalling("meow!")).toString)
        eventually(f.toString shouldEqual Future.failed(new GenericFailureCalling("meow!")).toString)
        eventually(f.toString shouldEqual Future.failed(new GenericFailureCalling("meow!")).toString)
        ...
        eventually(f.toString shouldEqual Future.failed(new GenericFailureCalling("No of retries exhausted")).toString)
      }
    }
    ...
    "calling callWithRetryWithoutArgs for function returning a successful future" should {
      "call function successfully without retry" in {
        val retryHandler = new RetryHandler
        val f = retryHandler.callWithRetry[String](serviceCallSuccess)(uriSuccess)
