...
class ArtistInfoServiceTest extends WordSpec with PrivateMethodTester with Matchers with MockitoSugar {
  ...
  "calling validateArtistInfoResponse with an OK status" should {
    "return ArtistInfo results successfully" in {
      ...
      result.toString shouldBe Future.successful(ArtistInfos(Metadata(0, 2, 2), Seq(ArtistInfo("Mark Hamill", 2, "https://image.tmdb.org/t/p/w185/fk8OfdReNltKZqOk2TZgkofCUFq.jpg", Seq(11, 181808), 2)))).toString
    }
  }

  "calling validateArtistInfoResponse with an 500 status" should {
    "return a validation error" in {
      val resultFailure = validateResult[ArtistInfos](500,"""{"bla"}""", decoder)
      resultFailure.toString shouldEqual Future.failed(new GenericFailureCalling("Exception calling artists endpoint")).toString
    }
  }
}
